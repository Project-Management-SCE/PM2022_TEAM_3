...
class BasicTests(TestCase):
    ...
    def test_address(self):
        ...
        self.assertFalse(
            len(acc.address) <= 50, "Check name is less than 50 digits long1"
        )
        acc.address = "One Apple Park Way, Cupertino, CA 95014, United States"
        self.assertTrue(
            len(acc.address) > 50, "Check name is less than 50 digits long2"
        )
...
class InsertInfoTest(BaseTest):
    ...
    def test_password_incorrect(self):
        response = self.client.post(
            self.login_url, self.user_unmatching_password, format="text/html"
        )
        self.assertTemplateUsed(response, "registration/login.html")
        self.assertEqual(response.status_code, 200)
    ...
    def test_user_incorrect(self):
        response = self.client.post(
            self.login_url, self.unmatching_user, format="text/html"
        )
        self.assertTemplateUsed(response, "registration/login.html")
        self.assertEqual(response.status_code, 200)
...
class Integrate_tests(TestCase):
    ...
    def test_Log_in_out(self):
        self.credentials = {"username": "testuser", "password": "userpass"}
        user = User.objects.create_user(**self.credentials)
...
class View_test(TestCase):
    ...
    def test_SearchUserByID_POSTLen(self):
        ...
        self.acc = Accounts.objects.create(
            user=self.user, is_doggiesitter=False, id="1"
        )
        self.user.save()
        self.acc.save()
        request = HttpRequest()
    ...
    def test_change_get(self):
        ...
        response = self.client.get(
            reverse("changeinfo", kwargs={"user_id": self.user.id}),
            data="",
            follow=True,
        )
        self.assertEqual(response.context["check"], "yes get")

    @tag("Unit-Test")
    def test_change_post_valid(self):
        self.credentials = {
            "username": "testuser",
            "password": "userpassdskfldskf",
            "first_name": "test",
        ...
        self.assertEqual(response.context["check"], "yes post")

    @tag("Unit-Test")
    def test_change_post_notvalid(self):
        self.credentials = {
            "username": "testuser",
            "password": "userpassdskfldskf",
            "first_name": "test",
...
class Admin_test(TestCase):
    ...
    def test_make_admin(self):
        ...
        response = self.client.post(
            reverse("admin_actions/make_admin"), data=form_data, follow=True
        )
        self.assertEqual(
            response.context["result"], "Admin permissions was successfully granted."
        )
    ...
    def test_delete_admin(self):
        ...
        form_data = {"selected_id": self.user.username}
        response = self.client.post(
            reverse("admin_actions/remove_admin"), data=form_data, follow=True
        )
        self.assertEqual(
            response.context["result"], "Admin permissions was successfully denied."
        )
    ...
    def test_Approve_Doggie(self):
        ...
        form_data = {"selected_id": self.user.username}
        response = self.client.post(
            reverse("admin_actions/approve_doggiesitter"), data=form_data, follow=True
        )
        self.assertEqual(
            response.context["result"], "Doggiesitter was successfully approved."
        )
